...
from utils.utils import read_input, save_output_to_csv, read_list_from_csv, cleanup_dataframe
...
teams_with_dl_results = pd.merge(transfermarkt, soccerway, how="left", left_on=["Team", "Year"],
                                 right_on=["Team", "Year"])

countries_map = countries_codes()

uefa['Country'] = uefa['CountryCode'].map(countries_map)
uefa_teams = cleanup_dataframe(
    uefa[uefa['Country'].isin(similar_leagues)].drop_duplicates(subset=["Team"])[["Team", "Country"]])

save_output_to_csv(uefa_teams.sort_values(by=["Country", "Team"]), "uefa_teams_names")
save_output_to_csv(
    transfermarkt.drop_duplicates(subset=["Team"])[["Country", "Team"]].sort_values(by=["Country", "Team"]),
    "transfermarkt_teams_names")
